...
def attempt(params, fixed, loss_func, num_iter, lr, lr_decay, eps, tol, verbose):
    ...
    pbar = tqdm(total=num_iter, unit='batches') if verbose else None
...
def optimise(param_shapes, loss_func, num_iter, lr, lr_decay, eps, tol,
             dtype=torch.float, verbose=False):
    ...
    success = attempt(params, fixed, loss_func, num_iter, lr, lr_decay, eps, tol, verbose)
    if success:
        return params
    return None
...
def rationalise(params, loss_func, num_iter, lr, lr_decay, eps, tol, denominators,
                verbose=False):
    ...
    for d in denominators:
        ...
        while any([(fixed[param_id].isnan() & ~used[param_id]).any() for param_id in range(len(params))]):
            ...
            try_round(params, target, fixed, used, loss_func, num_iter, lr, lr_decay, eps, tol, verbose)
